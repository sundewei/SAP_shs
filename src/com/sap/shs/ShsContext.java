...
public class ShsContext {
    ...
    public static final String JOB_ID_KEYWORD = "mapred.JobClient: Running job: ";
    public static final String JOB_EXCEPTION_KEYWORD = "Exception in thread";
...
//targetAddress = targetAddress.replace("www.amazon.com", "72.21.214.128");
        DefaultHttpClient httpclient = new DefaultHttpClient();
        System.out.println("Trying to hit : " + targetAddress);
    ...
    public static String getJsonpString(String callbackFn, Map<String, String> map) {
        StringBuilder sb = new StringBuilder();
        sb.append("{");
        for (Map.Entry<String, String> entry : map.entrySet()) {
            sb.append("\"").append(entry.getKey()).append("\":\"").append(entry.getValue()).append("\",");
    ...
    private static LoginPass getRequestLoginPass(Map<String, String[]> requestParaMap)
        ...
        System.out.println("\n\n\nrequestParaMap.get(" + EMPLOYEE_ID + ")=" + requestParaMap.get(EMPLOYEE_ID));
    ...
    private static String findPassword(String username) {
        if (username == null || username.length() == 0) {
            ...
            if ("deepflow".equals(username)) {
                return username;
            } else if ("hadoop".equals(username)) {
                return "abcd1234";
            } else if ("I123456".equals(username)) {
                return "hadoopsap";
            }
        }
        return null;
    ...
    public static void writeToHdfsFile(ConfigurationManager cm, String filename, String content) throws Exception {
        OutputStream out = cm.getFileSystem().getOutputStream(filename);
        IOUtils.write(content, out);
        out.close();
    }

    public static String readHdfsFile(ConfigurationManager cm, String filename) throws Exception {
        InputStream in = cm.getFileSystem().getInputStream(filename);
    ...
    protected static String getCommand(String scriptFilename,
        ...
        return getCommand(scriptFilename, new String[]{resourceFile, className, additionalParameters});
    }
    */
    protected static String getCommand(String scriptFilename, String... params) {
        StringBuilder command = new StringBuilder();
        command.append(scriptFilename).append(" ");
        for (String param : params) {
            command.append(param.replace(".clsss", "")).append(" ");
        }
