...
public class ListHdfsFolder extends BaseServlet {
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String callbackFs = null;
        HttpSession session = request.getSession(true);
        ConfigurationManager configurationManager = null;
        boolean jsonp = false;

        if (request.getServletPath().contains("/xs/")) {
            callbackFs = request.getParameter("callback");
            configurationManager = new ConfigurationManager("lroot", "abcd1234");
            jsonp = true;
        } else {
            configurationManager = ((LoginPass) session.getAttribute(ShsContext.LOGIN_PASS)).getConfigurationManager();
        }
        ...
        if (!jsonp) {
            ...
            out.write(ShsContext.getJsonpString(callbackFs, jsonKeyValMap));
        }
        out.close();
    }
